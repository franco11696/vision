...
def selection(img, i):
    ...
    while True:
        ...
        k = cv2.waitKey(1)
        if len(pts) == i: #3/4 points must be selected before ending if iteration depending on affine or homograf.
            break
...
img2 = cv2.resize(cv2.imread('pic3.jpg'), img.shape[1::-1]) #Load the 3rd picture while resizing it and inversing the tuple's order and cropping the last position in order to get w x h.
...
while(run == True):
    ...
    if k == ord('h'):
        cv2.destroyAllWindows()
        origin = selection(backup2.copy(),4)  #We pass the image and receive the tupple with the point's coordinates.
        final = np.array(vector, dtype=np.float32)
        img = homo(img2, origin, final) #Here we send the source and destiny points to perspective function.
        cv2.imwrite('homografia.png', img)          
    if k == ord('q'):
        run = False
    elif k==27:
        break
