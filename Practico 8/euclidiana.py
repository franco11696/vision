...
def homo(image, origin, final):
    (h, w) = image.shape[:2]    #Defines the matrix's size.
    z = cv2.getPerspectiveTransform(origin, final) #This function returns an affine transform calculated from three pairs of coodinates using a 2x3 transformation matrix.
    homo = cv2.warpPerspective(image, z, (w, h)) #warpAffine transforms an image using a matrix based on the given parameters (image=source, z=transf. matrix, h/w=size of output pic).
    return homo     